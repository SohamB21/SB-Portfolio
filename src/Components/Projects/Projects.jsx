...
const generateProjectItem = (project) => (
  <div className="project relative group my-8 mx-10">
    <img src={project.imageSrc} alt={project.altText} className="w-96 h-56 shadow-xl shadow-blue-500 rounded-lg transition-transform transform group-hover:scale-105 filter group-hover:blur-sm" />
    <div className={`info absolute inset-0 hidden group-hover:flex flex-col items-center justify-center text-center text-${project.textColor}`}>
      <h3 className="text-2xl font-bold m-2">{project.altText}</h3>
      <p>A brief description of the project.</p>
      <div className="flex flex-row items-center justify-center gap-6">
        {project.deployedLink && (
          <a href={project.deployedLink} className="mt-2 bg-blue-600 border border-blue-900 rounded-lg py-2 px-4" target="_blank" rel="noopener noreferrer">Deployed Project</a>
        )}
        {project.githubLink && (
          <a href={project.githubLink} className="mt-2 bg-blue-600 border border-blue-900 rounded-lg py-2 px-4" target="_blank" rel="noopener noreferrer">GitHub</a>
        )}
      </div>
    </div>
  </div>
...
const Projects = () => (
    <section className="s h-full w-full flex flex-col items-center justify-center py-20" style={{ background: 'linear-gradient(to right, white, lightblue)' }}>
