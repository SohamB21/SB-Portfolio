...
const TechnologySection = ({ title, technologies }) => {
  ...
  return (
    <div className="h-full flex flex-col items-center justify-center py-2" ref={ref}>
        ...
        {technologies.map((tech, index) => (
          <Grow key={tech.name} in={inView} timeout={index * 500}>
            <div className="relative group">
              <img
                className="h-16 m-1 shadow-md rounded-xl transition-transform transform group-hover:scale-110"
                src={tech.logo}
                alt={`${tech.name} logo`}
              />
...
const Skills = () => (
  <div className="s h-full w-full flex-1 items-center justify-center py-40" style={{ background: 'linear-gradient(to right, white, lightblue)' }}>
    <div className="h-full flex flex-col items-center justify-center">
      <h1 className="text-3xl font-bold mb-10">My Skills</h1>
      <TechnologySection title="Programming Languages" technologies={technologies.slice(0, 4)} />
      <TechnologySection title="Frontend Technologies" technologies={technologies.slice(4, 10)} />
      <TechnologySection title="Backend Technologies" technologies={technologies.slice(10, 14)} />
      <TechnologySection title="Mobile App Development" technologies={technologies.slice(14, 16)} />
      <TechnologySection title="Version Control Tools" technologies={technologies.slice(16, 18)} />
      <TechnologySection title="Technologies I'm Familiar With" technologies={technologies.slice(18)} />
    </div>
  </div>
);
